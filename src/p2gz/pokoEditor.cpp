...
{
	pokos = static_cast<RangeMenuOption*>(p2gz->menu->get_option("items/pokos"));
}

void PokoEditor::set_pokos(s32 pokos)
{
	if (Game::playData->mPokoCount >= 10000 && pokos < 10000) {
		Game::playData->mStoryFlags &= ~Game::STORY_DebtPaid;
	} else if (Game::playData->mPokoCount < 10000 && pokos >= 10000) {
		Game::playData->mStoryFlags |= Game::STORY_DebtPaid;
	}
...
void PokoEditor::update()
{
	...
	pokos->set_selection(Game::playData->mPokoCount);
}
