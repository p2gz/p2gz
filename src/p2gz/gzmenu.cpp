...
void GZMenu::init_menu()
{
    ...
    root_layer = (new ListMenu())
        ...
        ->push(new OpenSubMenuOption("captain", (new ListMenu(new Delegate1<NaviTools, MenuLayer*>(p2gz->navi_tools, &NaviTools::sync)))
			->push(new ToggleMenuOption("boing mode", false, new Delegate1<NaviTools, bool>(p2gz->navi_tools, &NaviTools::set_boing_mode)))
			->push(new FloatRangeMenuOption("health", 0.0f, 50.0f, 50.0f, new Delegate1<NaviTools, f32>(p2gz->navi_tools, &NaviTools::set_active_navi_hp)))
