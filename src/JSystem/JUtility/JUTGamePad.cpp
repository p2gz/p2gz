...
u32 JUTGamePad::read()
{
	...
	if (p2gz != nullptr && p2gz->mIsRecordingInputs && p2gz->mCurrentInputFrame < 5400) {
		p2gz->mControllerInputs[p2gz->mCurrentInputFrame] = ControllerInput(mPadStatus[0], mPadButton[0], mPadMStick[0], mPadSStick[0]);
		p2gz->mCurrentInputFrame++;
	}

	if (p2gz != nullptr && p2gz->mIsReplayingInputs && p2gz->mCurrentInputFrame < 5400) {
		ControllerInput input = p2gz->mControllerInputs[p2gz->mCurrentInputFrame];
		mPadStatus[0] = input.mPadStatus;
		mPadButton[0] = input.mPadButton;
		mPadMStick[0] = input.mPadMStick;
		mPadSStick[0] = input.mPadSStick;
