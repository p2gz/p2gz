...
if [ $# -eq 0 ] || [ "$drop" = "$1" ]; then
    echo "Please pass the filepath to a Pikmin 2 USA v1.00 *.iso file as an argument"
...
if [ $? -eq 1 ]; then
    echo "chmod +x'ing nodtool.macos failed."
    exit
fi
...
if [ -f "$1" ]; then
	...
	cd $home
	./nodtool.macos makegcn "$drop/root" "$drop/p2gz-$version.$ext"
	rm -r "$drop/root"
	echo "done! :)"
	read
else
