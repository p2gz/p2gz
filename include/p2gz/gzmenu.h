...
public:
	...
	~DecimalInputOption() { delete keypad; }
...
public:
	...
	{
		while (options.len() > 0) {
			Vec<MenuOption*>* row = options.pop();
			while (row->len() > 0) {
				MenuOption* opt = row->pop();
				delete opt;
			}
			delete row;
		}
...
public:
	...
	~GZMenu()
	{
		// `layer` is expected to be further down in the tree from `root_layer`
		// so it shouldn't need to be destroyed separately.
		delete root_layer;
	}
